...
数学的正しさの初期監査で省略されていた論証を展開し、既存の指数群リポジトリから
適合する恒等式、自由群の座標・collecting、モデル理論の証明をコピーして再利用した。
隣接リポジトリへの path dependency はない。元の同時 quotient、自由表示、非斉次の関係、
canonical な graded 写像と符号、任意 rank、有限 support、欠損商の基底を保った。
